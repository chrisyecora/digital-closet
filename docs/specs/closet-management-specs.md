...
| **CLOSET-DB-001** | **When** multiple distinct photos are logged, the system shall increment the worn_count for an item independently for each photo. Processing is per-photo and idempotent via the queue/backend; there is no daily wear count cap. | [ ] |
